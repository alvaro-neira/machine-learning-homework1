...
(X_train, X_test, y_train, y_test) = sklearn.model_selection.train_test_split(olivetti_faces.iloc[:, :-1].to_numpy(),
                                                                              olivetti_faces.iloc[:, -1:].to_numpy(),
                                                                              test_size=0.3)
...
def cross_val_score(**kwargs):
    ...
    cv = sklearn.model_selection.cross_validate(
        # Esta función entrena un modelo para distintos subconjuntos generados al azar.
